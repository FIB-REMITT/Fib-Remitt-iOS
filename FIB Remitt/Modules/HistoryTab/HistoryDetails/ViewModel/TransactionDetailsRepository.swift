...
class TransactionDetailsRepository {
  ...
  //  let vm = AuthViewModel()
    func transactionDetailsApi(transactionNumber: String)  {
        APIManager.shared.getData(endPoint: TransactionDetailsEndpoint.TransactionDetails(transactionNumber: transactionNumber), resultType: TransactionDetailsResponse.self, showLoader: true)
            .sink { completion in
                switch completion{
