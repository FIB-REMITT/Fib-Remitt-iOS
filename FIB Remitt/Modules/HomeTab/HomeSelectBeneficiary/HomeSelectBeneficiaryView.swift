            ...
            if type == .BankTransfer{
                VStack {
                    ForEach(beneficiaryVM.bankBeneficiaries ?? [], id: \.id) { beneficiary in
                        ...
                        AccountInfoCellView(selected: selected, title: beneficiary.fullName ?? "", subtitle1:"IBAN: \(beneficiary.accountNumber ?? "")" , subtitle2: beneficiary.bankBeneficiaryBankDTO?.name ?? "", icon: beneficiary.typeOfBeneficiary?.lowercased() == "personal" ? "personal_ico" : "business_ico",type: homeData.collectionPoint.lowercased() == "bank" ? .BankTransfer : .CashPickup)
                            .onTapGesture{
                ...
                VStack {
                    ForEach(beneficiaryVM.cashPickUpBeneficiaries ?? [], id: \.id) { beneficiary in
                        let selected = !resetSelection && (beneficiary.id == selectedBeneficiaryID)
                        AccountInfoCellView(selected: selected, title: beneficiary.fullName ?? "", subtitle1: "IBAN: \(beneficiary.phoneNumber ?? "")" , subtitle2:  beneficiary.address ?? "", icon: beneficiary.typeOfBeneficiary?.lowercased() == "personal" ? "personal_ico" : "business_ico",type: homeData.collectionPoint.lowercased() == "bank" ? .BankTransfer : .CashPickup)
                            .onTapGesture{
                                if  beneficiary.typeOfBeneficiary?.lowercased() == "business"{
                                    isFilePickerPresented = true
                                    selectedBeneficiaryID = beneficiary.id
